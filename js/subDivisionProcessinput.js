const subDivisionInput = JSON.parse(
	localStorage.getItem('subDivisionArrayData')
);
...
const createArrOfObservation = (columnNumber) => {
	...
	for (let i = 0; i < 31; ++i) {
		if (obsArr[i] === 'Dry' && forecastArr[i] === 'Dry') a++;
		else if (obsArr[i] === 'Dry' && forecastArr[i] === 'Isolated')
			b++;
		else if (
			obsArr[i] === 'Dry' &&
			forecastArr[i] === 'Scattered'
		)
			c++;
		else if (
			obsArr[i] === 'Dry' &&
			forecastArr[i] === 'Fairly Widespread'
		)
			d++;
		else if (
			obsArr[i] === 'Dry' &&
			forecastArr[i] === 'Widespread'
		)
			e++;
...
const PC = (array) => {
	...
	let number = Number(
		((array[0] + array[6] + array[12] + array[18] + array[24]) /
			total) *
			100
	).toFixed(3);
