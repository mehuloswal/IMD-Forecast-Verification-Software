...
//Frequency of Index
const FBI = (arr) =>
	Number(((arr[0] + arr[1]) / (arr[0] + arr[2])).toFixed(3));
//False alarm ratio
...
const CSI = (arr) =>
	Number((arr[0] / (arr[0] + arr[1] + arr[2])).toFixed(3));
//Equitable Threat Score
const ETS = (arr) => {
	let ar = ((arr[0] + arr[1]) * (arr[0] + arr[2])) / 31;
	return Number(
		((arr[0] - ar) / (arr[0] + arr[1] + arr[2] - ar)).toFixed(3)
	);
};
//Heidke Skill Score
const HSS = (arr) => {
...
const rainfallInput = JSON.parse(
	localStorage.getItem('rainfallArrayData')
);
...
const createArrOfObservation = (columnNumber) => {
	...
	for (let i = 0; i < 31; ++i) {
		if (obsArr[i] === 'Yes' && forecastArr[i] === 'Yes') hits++;
